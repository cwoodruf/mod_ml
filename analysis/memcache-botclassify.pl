...
my %opt;
getopts('m:l:p:',\%opt);
my $log = $opt{l} || "/srv/cal/src/apache/ml_logs/memcache-botclassify.log";
system "touch $log";
die "invalid log $log" unless -f $log;
my $port = $opt{p} || '49999';
die "invalid port $port" unless $port =~ /^\d{0,5}$/;
my $memcachehost = $opt{m} || '127.0.0.1:11211';
my $mc = Memcached::Client->new({servers => [$memcachehost]});
...
while(1)
{
    ...
    print LOG "received ip: $ip from data: $data\n";
    print "received ip: $ip from data: $data\n";
    my $prediction = $mc->get($data);
    my $response = "";
    ...
    print "received ip: $ip got: $prediction sending: $response data: $data\n";
